...
import { collection, addDoc, updateDoc, getDocs, doc, get } from "firebase/firestore";
...
export default function EditHamkor({ fid, closeEvent }) {
  ...
  useEffect(() => {
    console.log("FID" + fid.tel);
    setFio(fid.fio);
    setUserId(fid.user_id);
    setTel(Number(fid.tel));
    setLoc(fid.loc);
  }, [])
  ...
  const createuser = async () => {
    ...
    const userDoc = doc(db, "azizbek/azizbek-infos/hamkorlar", fid.id);
    const newFields = {
        fio: fio,
        user_id: user_id,
        loc: loc,
